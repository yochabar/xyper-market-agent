export async function requestJson(url, { method = 'GET', body, token, timeoutMs = 20000 } = {}) {
...
export function apiPost(config, path, body, token = '') {
  return requestJson(`${config.apiBase}${path}`, { method: 'POST', body, token });
}

export function apiGet(config, path, token = '') {
  return requestJson(`${config.apiBase}${path}`, { token });
}
...
export async function verifyXyper(config) {
  const [health, remoteConfig] = await Promise.all([
    requestJson(`${config.apiBase}/api/agent/v1/health/`),
    requestJson(`${config.apiBase}/api/agent/v1/config/`)
  ]);
